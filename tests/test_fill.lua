...
local function test_fill_commits_to_renderer()
    ...
    t.end_fill()
    h.drain(t)
    assert(#t.fills == 1, "expected 1 fill, got " .. #t.fills)
...
local function test_fill_vertex_count()
    ...
    assert(#t.fills[1].vertices == 4, "expected 4 vertices, got " .. #t.fills[1].vertices)
...
local function test_fill_requires_three_vertices()
    ...
    t.end_fill()
    h.drain(t)
    assert(#t.fills == 0, "fill with fewer than 3 vertices should not commit")
...
local function test_fill_inactive_after_end_fill()
    ...
    assert(#t.fills == 1, "should still be exactly 1 fill after end_fill")
...
local function test_overlapping_begin_fill_resets_vertices()
    ...
    t.end_fill()
    h.drain(t)
    assert(#t.fills == 1, "expected 1 fill after overlapping begin_fill")
    -- vertices should be from the second begin_fill only
    assert(#t.fills[1].vertices == 3,
        "expected 4 vertices from second begin_fill, got " .. #t.fills[1].vertices)
...
local function test_clear_resets_fills()
    ...
    t.clear()
    h.drain(t)
    assert(#t.fills == 0, "fills should be empty after clear")
...
local function test_reset_resets_fills()
    ...
    t.reset()
    h.drain(t)
    assert(#t.fills == 0, "fills should be empty after reset")
