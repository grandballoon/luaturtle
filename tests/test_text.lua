...
local function test_text_appends_to_log()
    ...
    t.text("hello")
    h.drain(t)
    assert(#t.texts == 1, "expected 1 text entry, got " .. #t.texts)
...
local function test_text_align_right()
    ...
    h.drain(t)
    assert(t.texts[1].align == "right", "align should be 'right'")
...
local function test_text_content_stored()
    ...
    t.text("hello world")
    h.drain(t)
    assert(t.texts[1].text == "hello world", "text content should be stored verbatim")
...
local function test_text_tostring_coercion()
    ...
    t.text(42)
    h.drain(t)
    assert(t.texts[1].text == "42", "numeric arg should be coerced to string '42'")
...
local function test_text_multiple_entries()
    ...
    t.text("third")
    h.drain(t)
    assert(#t.texts == 3, "expected 3 text entries, got " .. #t.texts)
    assert(t.texts[1].text == "first",  "entry 1 should be 'first'")
    assert(t.texts[2].text == "second", "entry 2 should be 'second'")
    assert(t.texts[3].text == "third",  "entry 3 should be 'third'")
...
local function test_text_is_instant()
    ...
    assert(#t.texts == 1, "text should commit in a single update call")
...
local function test_clear_removes_texts()
    ...
    t.clear()
    h.drain(t)
    assert(#t.texts == 0, "texts should be empty after clear()")
...
local function test_reset_removes_texts()
    ...
    t.reset()
    h.drain(t)
    assert(#t.texts == 0, "texts should be empty after reset()")
...
local function test_text_survives_after_clear()
    ...
    t.text("after")
    h.drain(t)
    assert(#t.texts == 1, "expected 1 text entry after clear + new text")
    assert(t.texts[1].text == "after", "surviving entry should be 'after'")
