...
local function test_setx_draws_segment_when_pen_down()
    ...
    t.setx(100)
    h.drain(t)
    assert(#t.segments > 0, "setx with pen down should produce a segment")
...
local function test_setx_no_segment_when_pen_up()
    ...
    t.setx(100)
    h.drain(t)
    assert(#t.segments == 0, "setx with pen up should produce no segment")
...
local function test_teleport_no_segment()
    ...
    t.teleport(100, 100)
    h.drain(t)
    assert(#t.segments == 0, "teleport should not produce a segment")
...
local function test_teleport_does_not_add_fill_vertex()
    ...
    t.end_fill()
    h.drain(t)
    assert(#t.fills == 1, "fill should still commit")
    -- vertices: start + forward + forward + forward = 4 (not 5, teleport excluded)
    assert(#t.fills[1].vertices == 4,
        "teleport should not add a fill vertex, got " .. #t.fills[1].vertices)
